...
| 🔄 **账号会话同步** | 把源 Profile 的登录态（Cookies、Local/Session Storage、IndexedDB、账号与同步数据等）复制到目标 Profile，保留目标原有扩展安装状态，不继承来源扩展记录；带变更预览、暂停 / 取消、覆盖前自动备份 |
| 🧩 **扩展扫描与迁移** | 识别扩展来源（Web Store / 本地 / Profile 内）、版本和数据目录，按浏览器界面语言显示扩展名称，迁移前为目标创建备份 |
| 🤖 **CDP 自动化入口** | 一键创建 Agent 浏览器时默认同步登录态和可迁移插件，也可取消插件同步；隔离 Profile 可带 remote debugging 端口启动，并验证端口可达，给 Agent / Browser 工具直接接管 |
...
- **账号和扩展分开** —— 账号同步不会复制来源 Profile 的扩展安装记录、启停状态或可单独识别的插件数据；一键创建 Agent 浏览器默认会额外同步可迁移插件，用户也可以取消勾选只同步登录态。
